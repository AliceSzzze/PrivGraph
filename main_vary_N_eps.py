...
def run_experiment(exper:int, run:Run):
    ...
    data_col = [run.epsilon,exper,run.n1,nmi,evc_overlap,evc_MAE,deg_kl, \
...
def main_vary_N(dataset_name='Chamelon',epsilon=2,e1_r=1/3,e2_r=1/3,N_List=[10,20],exp_num=10,save_csv=False, mat0=None, mat0_graph=None):
    ...
    run = Run(dataset_name, epsilon, e1_r * epsilon, e2_r * epsilon,
              e3_r * epsilon, exp_num, mat0, mat0_graph)

    run.cols = ['eps','exper','N','nmi','evc_overlap','evc_MAE','deg_kl', \
    'diam_rel','cc_rel','mod_rel']
    ...
    run.mat0_evc_a = dict(sorted(run.mat0_evc.items(),key = lambda x:x[1],reverse=True))
